...
[![DOI](https://zenodo.org/badge/DOI/10.5281/zenodo.3759707.svg)](https://doi.org/10.5281/zenodo.3759707)
...
The concept DOI [10.5281/zenodo.3759707](https://doi.org/10.5281/zenodo.3759707)
always resolves to the most recent version, and every version keeps its own DOI.
The current one is
[10.5281/zenodo.21902402](https://doi.org/10.5281/zenodo.21902402) (v0.1.0);
[10.5281/zenodo.3759708](https://doi.org/10.5281/zenodo.3759708) is the v0.0
placeholder cited in the paper, which holds a README and no code.

**The GitHub integration on this repository does not work.** Zenodo's stored
OAuth credential for GitHub returns 401, and re-linking returns 400 — the
repository was renamed after the 2020 deposit, and Zenodo's record of it still
carries the old name. Neither is reachable from the deposit API, so the script
below exists to make the integration unnecessary rather than to work around it
temporarily.

Zenodo's GitHub integration is meant to archive a release automatically. It is
also a single point of failure: the OAuth token Zenodo holds for GitHub can
expire, an organisation can restrict third-party applications, and the failure
shows up as a 401 or a 400 with nothing archived. `scripts/zenodo_release.py`
does the same job over the deposit API, where the only credential is your own:

```sh
export ZENODO_PAT=...                                    # zenodo.org > applications
git tag v0.2.0 && git push origin v0.2.0
uv run python scripts/zenodo_release.py --tag v0.2.0     # draft, nothing minted
uv run python scripts/zenodo_release.py --tag v0.2.0 --publish
```

It creates a new version of the existing record, drops the files inherited from
the previous one, uploads the GitHub archive of the tag, and applies
`.zenodo.json` with the version taken from the tag. Publishing mints a DOI and
cannot be undone, so it never publishes unless asked; without `--publish` it
stops at a draft and prints the URL.
